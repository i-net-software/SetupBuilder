...
#define DISABLED_OVERLAY_ALPHA ONE_HALF
...
-(float)scaleToAspectFit:(CGSize)source into:(CGSize)into paddingX:(float)paddingX paddingY:(float)paddingY;
@end
...
-(float)scaleToAspectFit:(CGSize)source into:(CGSize)into paddingX:(float)paddingX paddingY:(float)paddingY
{
    return MIN((into.width-paddingX) / source.width, (into.height-paddingY) / source.height);
}

-(NSFont*)fontSizedForAreaSize:(NSSize)size withString:(NSString*)string usingFont:(NSFont*)font
{
    CGSize sampleSize = [string sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:font, NSFontAttributeName, nil]];
    float scale = [self scaleToAspectFit:sampleSize into:size paddingX:20 paddingY:10];
    return [NSFont fontWithDescriptor:font.fontDescriptor size:scale * font.pointSize];
}
...
- (void)drawText:(NSString *)text withFrame:(NSRect)textFrame {
	...
	NSFont *sysFont = [self fontSizedForAreaSize:textFrame.size withString:text usingFont:[NSFont boldSystemFontOfSize:fontSize]];
...
- (void)tintBackgroundWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
	...
	if (self.showsOnOffLabels) {
		...
		leftSizeFrame.origin.x = (tracking ? thumbCenterX-(thumbFrame.size.width/2) : thumbFrame.origin.x) - cellFrame.size.width * TWO_THIRDS + 3;
		leftSizeFrame.origin.y = cellFrame.origin.y;
		leftSizeFrame.size.width = cellFrame.size.width * TWO_THIRDS + 3;
		...
		rightSizeFrame.origin.x = (tracking ? thumbCenterX+(thumbFrame.size.width/2) : thumbFrame.origin.x + thumbFrame.size.width) - 3;
		rightSizeFrame.origin.y = cellFrame.origin.y;
		rightSizeFrame.size.width = cellFrame.size.width * TWO_THIRDS - 3;
